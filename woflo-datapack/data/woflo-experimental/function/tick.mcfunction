...
scoreboard players remove @a[scores={wand_cd=1..}] wand_cd 1

# Reliable right-click detection (debug + route). We route if holding carrot_on_a_stick; router does detailed NBT routing.
execute as @a[scores={rclick=1..}] run title @s actionbar {"text":"[EXP] RClick detected","color":"yellow"}
execute as @a[scores={rclick=1..}] if entity @s[nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick"}}] run function woflo-experimental:wand_use_router
